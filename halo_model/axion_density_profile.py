...
def func_ax_halo_mass(M, cosmo_dic, power_spec_dic_sigma, M_cut, rho_central_param, hmcode_params, eta_given=False, axion_dic=None):
    ...
    if isinstance(M, (int, float)) == True:
        ...
        for i in range(len(M)):
            ...
            integral[i] = 4 * np.pi * integrate.simpson(y=func_dens_profile_ax(R_int, M[i], cosmo_dic, power_spec_dic_sigma, M_cut, rho_central_param[i], hmcode_params, eta_given=eta_given, axion_dic=axion_dic)*R_int**2, x=R_int)
